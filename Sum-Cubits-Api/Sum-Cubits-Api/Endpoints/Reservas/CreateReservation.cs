...
namespace Sum_Cubits_Api.Endpoints.Reservas
{
    public static class CreateReservation
    {
        ...
        [Authorize]
        public static async Task<IResult> Handle([FromBody] Request request,HttpContext httpContext, [FromServices] QueryReserva queryReservation, [FromServices] QueryUsuario queryUsuarios, [FromServices] UserService userService, [FromServices] QueryEstado queryEstado)
        {
            var userEmail = httpContext.User.FindFirstValue("email")
                         ?? httpContext.User.FindFirstValue(ClaimTypes.Email);

            var idEstado = await queryEstado.GetEstadoId("Confirmada");
